...
class Tile:
    ...
    def draw(self):
        ...
        if self.is_open:
            self.surface.blit(Tile.images[self.id], (self.x, self.y))
            pg.draw.rect(Tile.images[self.id], (0, 0, 0), pg.Rect(0, 0, Tile.size, Tile.size), 3)
        else:
            self.surface.blit(Tile.images[8], (self.x, self.y))
            pg.draw.rect(Tile.images[8], (0, 0, 0), pg.Rect(0, 0, Tile.size, Tile.size), 3)
...
class Game:
    ...
    def trigger(self, tile1, tile2):
        # It triggers the switch in each tile object.
    ...
    def display_debug_info(self):
        ...
        self.surface.blit(self.font.render(text, False, (0, 0, 0)), (self.font.get_height() / 2, self.height - self.font.get_height()))
    ...
    def draw(self):
        ...
        text_surface = self.game_font.render(str(int(self.curr_time / 1000)), False, (255, 255, 255))
    ...
    def update(self):
        ...
        self.curr_time += self.game_clock.tick(self.frames_per_second)
        # [[self.main_array[i][j].trigger() for j in range(4)]for i in range(4)]
