...
def partitioned_call(args, Tout, f, name=None):
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    ...
    Tout = [_execute.make_type(_t, "Tout") for _t in Tout]
    _, _, _op = _op_def_lib._apply_op_helper(
        "PartitionedCall", args=args, Tout=Tout, f=f, name=name)
    _result = _op.outputs[:]
    _inputs_flat = _op.inputs
    ...
    try:
      ...
      return partitioned_call_eager_fallback(
          args, Tout=Tout, f=f, name=name, ctx=_ctx)
    except _core._NotOkStatusException as e:
      if name is not None:
        message = e.message + " name: " + name
      else:
...
def partitioned_call_eager_fallback(args, Tout, f, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
...
def stateful_partitioned_call(args, Tout, f, name=None):
  r"""returns `f(inputs)`, where `f`'s body is placed and partitioned.
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    ...
    Tout = [_execute.make_type(_t, "Tout") for _t in Tout]
    _, _, _op = _op_def_lib._apply_op_helper(
        "StatefulPartitionedCall", args=args, Tout=Tout, f=f, name=name)
    _result = _op.outputs[:]
    if not _result:
      return _op
    _inputs_flat = _op.inputs
...
def stateful_partitioned_call_eager_fallback(args, Tout, f, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
...
def stateless_if(cond, input, Tout, then_branch, else_branch, name=None):
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    ...
    _, _, _op = _op_def_lib._apply_op_helper(
        ...
        then_branch=then_branch, else_branch=else_branch, name=name)
    _result = _op.outputs[:]
    _inputs_flat = _op.inputs
    _attrs = ("Tcond", _op.get_attr("Tcond"), "Tin", _op.get_attr("Tin"),
              "Tout", _op.get_attr("Tout"), "then_branch",
