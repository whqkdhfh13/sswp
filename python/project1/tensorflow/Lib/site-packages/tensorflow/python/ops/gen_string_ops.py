...
@deprecated_endpoints('as_string')
...
@deprecated_endpoints('regex_replace')
...
def string_format(inputs, template="%s", placeholder="%s", summarize=3, name=None):
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    ...
    _, _, _op = _op_def_lib._apply_op_helper(
        ...
        placeholder=placeholder, summarize=summarize, name=name)
    _result = _op.outputs[:]
    _inputs_flat = _op.inputs
...
@deprecated_endpoints('string_to_hash_bucket')
