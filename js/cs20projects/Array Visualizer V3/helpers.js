function drawArray(anArray, low, high, scale) {
	...
	for(var y = low; y <= high; y += scale) {
        yCoord = map(y, low, high, height - 10, 50);
        stroke(40);
        strokeWeight(2);
    ...
    for (var i = 0; i < anArray.length; i++) {
        var barHeight = map(anArray[i], low, high, 0, height - 60);
        rect(barWidth * i + 30, height - 10 - barHeight, barWidth, barHeight);
    }
...
function drawHzLine(val, low, high, col) {
    stroke(col);
    lineY = map(val, low, high, height - 10, 50);
